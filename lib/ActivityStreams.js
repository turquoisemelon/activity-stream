...
const {SyncProvider} = require("lib/SyncProvider");
...
function ActivityStreams(options = {}) {
  ...
  this._syncProvider = new SyncProvider({
...
ActivityStreams.prototype = {
  ...
  _processAndSendLinks(placesLinks, responseType, worker, options) {
    ...
    this._previewProvider.getLinkMetadata(placesLinks, event, skipPreviewRequest, previewsOnly).then(linksToSend => {
      this._syncProvider.syncLinks(responseType, linksToSend).then((syncedLinks) => {
        this.send(am.actions.Response(responseType, syncedLinks, {append}), worker);
      });
    });
  },
